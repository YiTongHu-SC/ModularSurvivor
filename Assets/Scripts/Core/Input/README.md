...
public class PlayerMovement : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 注意：使用 Space.World 确保移动方向是世界坐标系
    }
}
```
...
```csharp
// 原始输入: input.x (左右), input.y (前后)
// 相机变换: Right = 相机X轴, Forward = 相机Z轴（忽略Y轴）
// 最终方向: cameraRight * input.x + cameraForward * input.y
```
