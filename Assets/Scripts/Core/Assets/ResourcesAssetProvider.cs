...
namespace Core.Assets
{
    ...
    public class ResourcesAssetProvider : IAssetProvider
    {
        ...
        public AssetHandle<T> Load<T>(AssetKey key, string scopeName = null) where T : UnityEngine.Object
        {
        ...
        public async Task<AssetHandle<T>> LoadAsync<T>(AssetKey key, string scopeName = null) where T : UnityEngine.Object
        {
            var cacheKey = GetCacheKey<T>(key);
            ...
            if (_handleCache.TryGetValue(cacheKey, out var cachedHandle))
            {
                ...
                // 等待加载完成
                while (handle.State == AssetLoadState.Loading)
                {
                    await Task.Yield();
                }
        ...
        public async Task<AssetHandle<T>[]> LoadBatchAsync<T>(AssetKey[] keys, string scopeName = null, IProgress<float> progress = null) where T : UnityEngine.Object
        {
            ...
            {
                tasks[i] = LoadAsync<T>(keys[i], scopeName);
            }
        ...
        public async Task<GameObject> InstantiateAsync(AssetKey key, Transform parent = null, string scopeName = null)
        {
            ...
            if (!handle.IsValid)
            {
                throw new InvalidOperationException($"Failed to load prefab for instantiation: {key}, Error: {handle.ErrorMessage}");
            }
            
            return UnityEngine.Object.Instantiate(handle.Asset, parent);
        }
        ...
        public AssetHandle<T> GetCachedHandle<T>(AssetKey key) where T : UnityEngine.Object
        {
            var cacheKey = GetCacheKey<T>(key);
        ...
        {
            var cacheKey = GetCacheKey<UnityEngine.Object>(key);
            return _handleCache.ContainsKey(cacheKey);
        }
        
        private string GetCacheKey<T>(AssetKey key) where T : UnityEngine.Object
        {
