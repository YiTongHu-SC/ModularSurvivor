...
namespace Core.Assets
{
    ...
    public class AssetSystem : IDisposable
    {
        ...
        public static AssetSystem Instance => _instance;
        ...
        public AssetSystem(AssetCatalog catalog)
        {
            ...
            _provider = new ResourcesAssetProvider(catalog);
        ...
        public AssetScope CreateScope(string name)
        {
            ...
            lock (_lockObject)
            {
                ...
                var scope = new AssetScope(name, _provider);
                _scopes[name] = scope;
                return scope;
            }
        }
        ...
        /// 加载清单中的所有资源到指定作用域
        /// </summary>
        public async Task<LoadManifestResult> LoadManifestAsync(AssetManifest manifest, string scopeName, IProgress<float> progress = null)
        {
            ThrowIfDisposed();
            ...
            for (int i = 0; i < entries.Length; i++)
            {
                ...
                try
                {
                    var handle = await scope.AcquireAsync<UnityEngine.Object>(key);
                    if (handle.IsValid)
                    {
                        result.SuccessCount++;
    ...
    public class LoadManifestResult
    {
        ...
        public List<AssetKey> LoadedKeys { get; } = new List<AssetKey>();
        public List<AssetKey> FailedKeys { get; } = new List<AssetKey>();
