...
namespace Core.Assets
{
    ...
    public static class AssetSystemUtils
    {
        ...
        public static AssetManifest CreateEnemyFamilyManifest(string familyName, string[] enemyKeys)
        {
            ...
            {
                entries.Add(new ManifestEntry(enemyKey, AssetType.Prefab, 1f, false, new[] { CommonTags.ENEMY, familyName }));
            }
            
            // 使用反射设置私有字段（仅用于运行时创建）
            var entriesField = typeof(AssetManifest).GetField("_entries", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
        ...
        /// 预热常用资源类型
        /// </summary>
        public static async System.Threading.Tasks.Task WarmupCommonAssets()
        {
            var assetSystem = AssetSystem.Instance;
