...
namespace Core.Coordinates
{
    ...
    public static class PathUtils2D
    {
        ...
        public static List<Vector2D> GetAvoidancePath(Vector2D start, Vector2D end, Area2D obstacle)
        {
            ...
            if (!IsPathBlocked(start, end, obstacle)) return GetStraightPath(start, end);

            // 简单的绕行策略：通过障碍物的边界点
            if (obstacle is CircleArea2D circle) return GetCircleAvoidancePath(start, end, circle);

            if (obstacle is RectArea2D rect) return GetRectAvoidancePath(start, end, rect);
        ...
        private static List<Vector2D> GetCircleAvoidancePath(Vector2D start, Vector2D end, CircleArea2D circle)
        {
            ...
            var toCenter = circle.Center - start;

            // 计算切点
            var perpendicular = new Vector2D(-toCenter.y, toCenter.x).Normalized;
            var tangentPoint1 = circle.Center + perpendicular * (circle.Radius + 0.5f);
            var tangentPoint2 = circle.Center - perpendicular * (circle.Radius + 0.5f);

            // 选择更接近目标的切点
            var dist1 = Vector2D.Distance(tangentPoint1, end);
            var dist2 = Vector2D.Distance(tangentPoint2, end);

            var chosenTangent = dist1 < dist2 ? tangentPoint1 : tangentPoint2;
        ...
        private static List<Vector2D> GetRectAvoidancePath(Vector2D start, Vector2D end, RectArea2D rect)
        {
            ...
            {
                var distance = Vector2D.Distance(corner, end);
        ...
        public static List<Vector2D> SmoothPath(List<Vector2D> originalPath, float smoothingFactor = 0.5f)
        {
            ...
            var smoothedPath = new List<Vector2D>();
            smoothedPath.Add(originalPath[0]); // 保持起点

            for (var i = 1; i < originalPath.Count - 1; i++)
            {
                var prev = originalPath[i - 1];
                var current = originalPath[i];
                var next = originalPath[i + 1];

                var smoothed = Vector2D.Lerp(current, (prev + next) * 0.5f, smoothingFactor);
        ...
        public static float GetPathLength(List<Vector2D> path)
        {
            ...
            var totalLength = 0f;
            for (var i = 1; i < path.Count; i++) totalLength += Vector2D.Distance(path[i - 1], path[i]);
        ...
        public static Vector2D GetPointAtDistance(List<Vector2D> path, float distance)
        {
            ...
            {
                var segmentLength = Vector2D.Distance(path[i - 1], path[i]);
