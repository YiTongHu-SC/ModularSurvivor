...
namespace Core.Assets
{
    ...
    public class MemoryMaintenanceService : MonoBehaviour
    {
        ...
        public static MemoryMaintenanceService Instance { get; private set; }

        public MemoryMaintenanceSettings Settings => _settings;
        public bool IsMaintenanceRunning { get; private set; }

        public int LevelSwitchCount { get; private set; }
        ...
        {
            if (!IsMaintenanceRunning) StartCoroutine(PerformMaintenanceCoroutine(0f));
        }
        ...
        private bool ShouldPerformMaintenance()
        {
            ...
            // 检查关卡切换次数
            if (LevelSwitchCount >= _settings.LevelSwitchThreshold)
                return true;
        ...
        private IEnumerator PerformMaintenanceCoroutine(float delay)
        {
            ...
            Debug.Log(
                $"[MemoryMaintenance] Starting maintenance. Memory: {startMemory:F2}MB, Level switches: {LevelSwitchCount}");
            ...
            Debug.Log(
                $"[MemoryMaintenance] Completed. Duration: {duration:F2}s, Memory: {endMemory:F2}MB (reduced by {memoryReduction:F2}MB)");
        }
        ...
        public MemoryStats GetMemoryStats()
        {
            return new MemoryStats
            {
                ...
                LevelSwitchCount = LevelSwitchCount,
    ...
    {
        [Header("Trigger Conditions")] [Tooltip("关卡切换多少次后触发维护")]
        public int LevelSwitchThreshold = 3;

        [Tooltip("内存使用量超过多少MB时触发维护")] public float MemoryThreshold = 500f;

        [Tooltip("距离上次维护多少秒后可以再次触发")] public float TimeInterval = 300f; // 5分钟

        [Header("Execution Settings")] [Tooltip("关卡切换后延迟多少秒执行维护")]
        public float DelayAfterLevelSwitch = 2f;

        [Tooltip("回到主界面后延迟多少秒执行维护")] public float DelayAfterReturnToMenu = 5f;

        [Tooltip("是否同时执行垃圾回收")] public bool EnableGarbageCollection = true;

        public static MemoryMaintenanceSettings Default => new();
    }
