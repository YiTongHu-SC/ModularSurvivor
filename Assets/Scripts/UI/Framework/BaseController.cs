...
namespace UI.Framework
{
        ...
        where TView : class
    {
        ...
        public virtual void Initialize(TModel model, TView view)
        {
            if (IsInitialized)
            {
                Debug.LogWarning($"BaseController<{typeof(TModel).Name}, {typeof(TView).Name}>: Controller already initialized!");
                return;
            }
            
            if (model == null || view == null)
            {
                Debug.LogError($"BaseController<{typeof(TModel).Name}, {typeof(TView).Name}>: Model or View cannot be null!");
                return;
            }
        ...
        public virtual void Start()
        {
            if (!IsInitialized)
            {
                Debug.LogError($"BaseController<{typeof(TModel).Name}, {typeof(TView).Name}>: Controller not initialized!");
                return;
            }
            
            if (IsRunning)
            {
                Debug.LogWarning($"BaseController<{typeof(TModel).Name}, {typeof(TView).Name}>: Controller already running!");
                return;
            }
        ...
        protected void SafeExecute(Action action, string actionName = "Unknown")
        {
            ...
            {
                Debug.LogWarning($"BaseController<{typeof(TModel).Name}, {typeof(TView).Name}>: Cannot execute {actionName} - controller not ready!");
                return;
            }
            ...
            {
                Debug.LogError($"BaseController<{typeof(TModel).Name}, {typeof(TView).Name}>: Error executing {actionName} - {ex.Message}");
            }
        }
        ...
        ~BaseController()
        {
            if (!IsDisposed)
            {
                Debug.LogWarning($"BaseController<{typeof(TModel).Name}, {typeof(TView).Name}>: Controller was not properly disposed!");
                Dispose();
            }
        }
    }
